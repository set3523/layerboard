...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", ({ roomId, role, userName }) => {
    ...
    // 🔥 다중 선생님 충돌 해결: 방에 있는 사람 중 '선생님 1명'만 찾아서 그 사람에게만 동기화 요청!
    ...
    if (roomClients) {
      for (const clientId of roomClients) {
        ...
        // 나 자신을 제외하고, 역할이 'teacher'인 사람을 딱 1명만 찾으면 바로 멈춤(break)
        if (clientSocket && clientSocket.role === "teacher" && clientId !== socket.id) {
    ...
    // 대표 선생님을 찾았다면, 그 선생님의 컴퓨터(소켓)에만 콕 집어서 요청함!
    if (masterTeacherId) {
  ...
  // 🚀 3. [신규] 선생님의 칠판 통제 명령 중계 차선! (수업모드, 투명도, 필기 끄기 등)
  socket.on("update-room-state", (newState) => {
    if (socket.roomId) {
      // 나(선생님)를 제외한 방 안의 모든 학생들에게 즉시 통제 명령 하달!

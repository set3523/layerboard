...
src/
  ...
  components/
    ...
    TeacherControlPanel   선생 패널(참가자·학생통제·단계·배경)
  ...
  hooks/
    ...
    useSteps  useTouchSwipe  useSingleSession
  lib/
...
**정의:** `lib/api/auth.ts` (signUp/signIn Email·Google·Kakao, `getCurrentUser`=`getSession().user`), `hooks/useSingleSession.ts`.

- 세션은 항상 `supabase.auth.getSession()` 사용 → 만료 시 자동 갱신(`getUser()`는 만료 시 로그인 풀림이라 안 씀).
- **단일 활성 세션:** 기기별 토큰을 `profiles.active_session`에 점유(`claim_session` RPC). 자기 `profiles` 행을 Realtime 구독 → 토큰이 바뀌면(=다른 기기 로그인) 강제 로그아웃. `dashboard`·`room` 페이지에서 `useSingleSession()` 마운트.
...
**정의:** `lib/api/rooms.ts` — `createRoom`(insert: code·teacher_id·is_active·max_students), `getTeacherRooms`, `deleteRoom`, `verifyRoomCode`(→ `get_room_by_code` RPC).
...
**저장:** `public.rooms` (id·code·teacher_id·is_active·max_students·created_at).
...
**저장:** 로컬 캔버스(Fabric) → 완료 시 broadcast + (선생) DB. **캐시:** 없음(실시간 계산).
...
| 이벤트 | 방향 | 용도 | 영구저장? |
|---|---|---|---|
| `draw` | 완료 획 | 원격 렌더 + (선생) DB 저장 | ✅ |
| `draw-live` | 그리는 중(50ms throttle, 같은 id 교체) | 실시간 스트리밍 | ❌ |
| `erase` | 지우기 id들 | 원격 제거 | (선생) ✅ |
| `update-room-state` | 선생→학생 | 수업모드·페이지 이동 강제 | — |
| `sync-steps` | 선생→학생 | 풀이 단계(표시/투명도) | — |
| `sync-bg` | 선생→학생 | 배경 투명도 | — |
| `sync-pages` | 선생→학생 | PDF/페이지 메타 실시간(새 PDF만 lazy 로드) | — |
| presence | 전원 track `{role,name,joinedAt}` | 정원(roomFull) + 선생 참가자 명단(roster) | — |
...
- **room_pages** = 페이지 스냅샷. **선생만** 자동저장(`saveRoomPages`, `pages` 변경 시 **1초 debounce**). `updateCurrentPageJson`이 전경필기(`json`)와 배경필기(`background.drawingsJson`)를 분리하고 **배경 이미지는 제외**.
- **room_events** = 이벤트 로그(`add`/`erase`/`page`) — 재생·역사용. `appendEvents`.
- 학생 필기도 **선생이 broadcast로 받아서** 자기 DB에 저장(`applyRemoteDraw` 내 `role==="teacher"`).
- 용량: `get_teacher_usage` RPC(events + pages + storage 바이트 합산) → `usage.canStore`로 초과 시 저장 중단(배너).

**저장:** `room_pages`, `room_events` (Postgres, jsonb).
**캐시:** `localStorage["room_{roomId}_state"]`(선생 방 상태: currentPageIndex·bgOpacity — 새로고침 복원).
...
**정의:** `lib/api/usage.ts`(`getAccountStatus`/`getTeacherPlan`/`getPlanStudentCap`), `lib/billing.ts`(가격·비례계산·주기), `lib/billingClient.ts`(결제창), `lib/toss.ts`(서버 Toss 호출), `app/billing/*`(라우트).
...
**RLS 원칙:**
- 선생(authenticated) = **자기 것만**(teacher_id/id = auth.uid()).
- 학생(anon) = **활성 방만** 조회(헬퍼 `is_active_room`), 이벤트 insert. rooms 직접조회 차단.
- 서버 라우트 = `service_role`(RLS 우회, billing).

**RPC(함수):** `is_active_room`, `get_room_by_code`, `get_teacher_usage`, `start_trial`, `claim_session`, `redeem_gift`, `handle_new_user`(가입 트리거).
...
| `room_{id}_state` | localStorage | 선생 방 상태(페이지·투명도) | 영구 |
...
- **DB 새 세팅/복구:** `db/schema.sql` 통째 실행(맨 위 DROP 주의 — 데이터 삭제).

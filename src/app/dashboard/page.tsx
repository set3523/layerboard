...
import { createRoom, getHostRooms, deleteRoom } from "@/lib/api/rooms";
...
import { roomPageOrientationKey, type PageOrientation } from "@/lib/pageFormat";

function DashboardPageContent() {
  const router = useRouter();

  // 상태 관리
  const [userName, setUserName] = useState("방장");
  ...
  const [newRoomPageOrientation, setNewRoomPageOrientation] = useState<PageOrientation>("portrait");
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        ...
        setUserName(user.email?.split("@")[0] || "방장");

        // 내가 만든 방 목록
        const myRooms = await getHostRooms(user.id);
        setRooms(myRooms || []);
        ...
        alert(formatUserFacingError(error));
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleCreateRoomClick = () => {
    ...
    setNewRoomPageOrientation("portrait");
    setShowCreateModal(true);
  };

  const handleCreateRoom = async () => {
    if (!userId) return;
    const pass = newRoomPassphrase.trim();
    if (pass.length < 4) {
      alert("입장 암호는 4자 이상으로 설정해 주세요.");
      return;
    }
    ...
    try {
      const newRoom = await createRoom(userId, pass);
      try {
        localStorage.setItem(roomPageOrientationKey(newRoom.code), newRoomPageOrientation);
      } catch {
        /* private mode 등 */
      }
      setShowCreateModal(false);
      window.alert(
        `교실이 만들어졌습니다.\n\n방 코드: ${newRoom.code}\n입장 암호: ${pass}\n\n학생에게 코드와 암호를 함께 알려 주세요.`
      );
      router.push(`/room/${newRoom.code}?role=host&name=${encodeURIComponent(userName)}`);
    } catch (e) {
    ...
    <div className="flex h-screen overflow-hidden font-sans text-gray-900 selection:bg-leaf-100">
        ...
        <header className="h-16 gg-glass border-b border-gray-200 px-8 flex items-center justify-between sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-gray-900">{userName} · 방장</p>
            </div>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            <div className="gg-glass p-8">
              ...
              <p className="text-gray-500 text-sm mb-6 leading-relaxed">
                방 코드와 입장 암호를 설정해 학생만 수업에 들어오게 할 수 있습니다.
              </p>
              <button
              ...
              >
                {isCreating ? <><Loader2 size={20} className="animate-spin" /> 생성 중...</> : "새 교실 만들기"}
              </button>
            </div>
            {/* 우측 카드: 저장 용량 사용량 게이지 */}
                ...
                {rooms.map((room) => (
                  <div key={room.id} className="gg-glass p-5 flex items-center justify-between hover:border-leaf-300 transition-colors group">
                    ...
                    <div className="flex items-center gap-1">
                      {/* ✨ 발급된 코드로 화이트보드 입장하기 */}
                      <button 
                        onClick={() => router.push(`/room/${room.code}?role=host&name=${userName}`)}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="gg-glass p-8 w-full max-w-sm">
            <h3 className="text-xl font-bold mb-2 text-gray-900">입장 암호 설정</h3>
            <p className="text-sm text-gray-500 mb-2">학생은 방 코드와 이 암호를 함께 입력해야 입장합니다. (4자 이상)</p>
            <p className="text-sm font-bold text-gray-700 mb-2">용지 크기 (A4)</p>
            <div className="flex gap-2 mb-4">
              <button
                type="button"
                onClick={() => setNewRoomPageOrientation("portrait")}
                className={`flex-1 py-3 rounded-xl text-sm font-bold border transition-colors ${
                  newRoomPageOrientation === "portrait"
              ...
              <button
                type="button"
                onClick={() => setNewRoomPageOrientation("landscape")}
                className={`flex-1 py-3 rounded-xl text-sm font-bold border transition-colors ${
                  newRoomPageOrientation === "landscape"
                    ? "border-leaf-500 bg-leaf-50 text-leaf-700"
                    : "border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300"
                }`}
              >
            ...
            <input
              ...
              placeholder="입장 암호"
              className="w-full p-4 mb-6 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-leaf-500 text-center font-bold"
              minLength={4}
              autoFocus
            />
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isCreating}
                className="gg-btn gg-btn--primary flex-1 disabled:opacity-70"
              >
                {isCreating ? "생성 중…" : "만들기"}
              </button>
            </div>

...
import { signInWithEmail, signUpWithEmail, signInWithSocial, safeRedirectPath } from "@/lib/api/auth";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        setIsLogin(true); // 가입 성공 시 로그인 화면으로 전환
        setIsLoading(false);
        return;
      }
          ...
          <div className="mt-8">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="w-full inline-flex justify-center items-center py-3 px-4 rounded-xl shadow-sm bg-[#FEE500] hover:bg-[#FDD800] text-sm font-bold text-[#000000] transition-all active:scale-[0.98]"
              >
                <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 3c-5.523 0-10 3.52-10 7.864 0 2.766 1.776 5.19 4.453 6.474l-1.127 4.195c-.1.373.315.65.653.44l4.9-3.23c.365.05.738.077 1.121.077 5.523 0 10-3.52 10-7.864C22 6.52 17.523 3 12 3z"/>
                </svg>
                Kakao
              </button>
            </div>

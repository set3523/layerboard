...
// 현재 로그인한 선생님의 총 사용량(바이트) = DB(이벤트+필기 스냅샷) + Storage(교재/캡처) 합산.
// RPC(get_teacher_usage)가 없으면 0으로 폴백 → 게이트가 막지 않음(안전).
export const getTeacherUsage = async (): Promise<number> => {
  const { data, error } = await supabase.rpc("get_teacher_usage");
  if (error) {
...
// 선생님 요금제. profiles에서 매번 최신값 읽음 (캐시 제거 — 업그레이드 즉시 반영).
export const getTeacherPlan = async (): Promise<Plan> => {
  try {
...
export const getUsageSummary = async (): Promise<UsageSummary> => {
  const [usedBytes, plan] = await Promise.all([getTeacherUsage(), getTeacherPlan()]);
